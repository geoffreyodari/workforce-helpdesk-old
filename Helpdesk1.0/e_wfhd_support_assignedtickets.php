...
if (isset($_POST['ticketid2'])){
	$from= $_POST['from'];
	$to= $_POST['to'];	
$query_obc = "SELECT * FROM `e_wfhelpdeskentries` WHERE idx LIKE '".$_POST['ticketid2']."' OR staffid LIKE '".$_POST['ticketid2']."' OR submitter LIKE '".$_POST['ticketid2']."'OR wstatus LIKE '".$_POST['ticketid2']."'AND wfAnalyst2 LIKE '". $alias ."' ORDER BY submitDate DESC LIMIT " . $offset . "," . $no_of_records_per_page ."";
$result = mysql_query($query_obc);
$sql_pagination = "SELECT * FROM `e_wfhelpdeskentries` WHERE idx LIKE '".$_POST['ticketid2']."' OR staffid LIKE '".$_POST['ticketid2']."' OR submitter LIKE '".$_POST['ticketid2']."'OR wstatus LIKE '".$_POST['ticketid2']."'AND wfAnalyst2 LIKE '". $alias ."' ORDER BY submitDate DESC";
	$result_pagination = $conn->query($sql_pagination);
	$total_rows=$result_pagination->num_rows;
	$total_pages = ceil($total_rows / $no_of_records_per_page);
}
...
{
	$query_obc = "SELECT * FROM `e_wfhelpdeskentries` WHERE  wfAnalyst2 LIKE '". $alias ."'  ORDER BY submitDate DESC LIMIT " . $offset . "," . $no_of_records_per_page ."";
	$result = mysql_query($query_obc);
	$sql_pagination = "SELECT * FROM `e_wfhelpdeskentries` WHERE  wfAnalyst2 LIKE '". $alias ."'  ORDER BY submitDate DESC";
	$result_pagination = $conn->query($sql_pagination);
	$total_rows=$result_pagination->num_rows;
	$total_pages = ceil($total_rows / $no_of_records_per_page);
}
  ...
  <div class="container-fluid">
    ...
    <div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
      <ul class="nav navbar-nav">
        ...
        <li class="active"><a ><span class="sr-only">(current)</span>My Tickets</a></li>
...
<ul class="pagination">
        <li><a href="<?php echo $support_assigned_tickets ?>?pageno=1">First</a></li>
        <li class="<?php if($pageno <= 1){ echo 'disabled'; } ?>">
            <a href="<?php if($pageno <= 1){ echo '#'; } else { echo $support_assigned_tickets  . "?pageno=".($pageno - 1); } ?>">Prev</a>
        </li>
        <li class="<?php if($pageno >= $total_pages){ echo 'disabled'; } ?>">
            <a href="<?php if($pageno >= $total_pages){ echo '#'; } else { echo  $support_assigned_tickets  . "?pageno=".($pageno + 1); } ?>">Next</a>
        </li>
        <li><a href="<?php echo $support_assigned_tickets  ?>?pageno=<?php echo $total_pages;  ?>">Last</a></li>
    </ul>
</div>
</div>
</div>
